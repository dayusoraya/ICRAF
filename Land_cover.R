...
nlb_sector<-subset(GDP_tables[18:35,2:12]) # AE: why you are using GDP from model?
...
growth_nlb.2021=GDP_nlb[2]+(GDP_nlb[2]*avg_nlb.rate) #AE = this is only for one year
...
for (i in 2:ncol(GDP_tables)) {
  ...
  GDPval<-GDP_tables[,i]+(GDP_tables[,i]*0.06*(3*(i-1))) # AE : what is the value of 0.06?
  GDP_Lin[[i-1]]=GDPval
